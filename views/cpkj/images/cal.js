var debug = -1; // -1: disable; 0: all; N: show level N debug msg.
...
function drawCld(SY,SM) {
   ...
   for(i=0;i<42;i++) {
      ...
      if(sD>-1 && sD<cld.length) {         
         ...
         if(cld[sD].isToday){
            //设置今天的背景色
            sObj.style.background = '#FFFFCC';
            sObj.style.fontWeight = 'bold';
            lObj.style.background = '#F8BD31';
            lObj.style.fontWeight = 'bold';
         }
         ...
         if(s.length>0) {
            ...
            if(s.length>0) {
               ...
               s=cld[sD].solarTerms;
               if(s.length>0) s = s.fontcolor('#FF6600');
            }
         }
...
function getdata_tm() {
    //http://www.cpzhan.com
    var current_y = $('#sy').val();
    var current_m = $('#sm').val();
    var cdate = current_y + current_m;

    $('#week td').each(function () {
        var this_day = $(this).text();
        var ball7 = '';
        $(this).removeClass("drawdate");
        if (jQuery.isArray(marksix[cdate]) && (ball7 = jQuery.inArray(this_day, marksix[cdate])) >= 0) {
            $(this).addClass("drawdate");
            if (n7[cdate] && n7[cdate][ball7])
                $(this).css("background", 'url(/views/cpkj/images/sm/' + n7[cdate][ball7] + '.gif) no-repeat top right');
            else
                $(this).css("background", 'url(/views/cpkj/images/marksix.gif) no-repeat top right');
